...
def function (grammar, name, prefix, anonymous=None):
	...
	s=grammar.symbols
	if anonymous:
		return g.rule(name, s.CheckIndent, prefix._as('type'), s.NOTHING._as('name'), g.agroup(s.EOL, g.arule(s.ParameterList.optional(), s.EOL))._as('parameters'), s.Decorator.zeroOrMore()._as('decorators'), s.Documentation.optional()._as('documentation'), s.Body._as('body'), s.OWhere.zeroOrMore()._as('where'))
	elif True:
		return g.rule(name, s.CheckIndent, prefix._as('type'), s.NameType._as('name'), g.agroup(s.EOL, g.arule(s.ParameterList.optional(), s.EOL))._as('parameters'), s.Decorator.zeroOrMore()._as('decorators'), s.Documentation.optional()._as('documentation'), s.Body._as('body'), s.OWhere.zeroOrMore()._as('where'))
...
def createProgramGrammar (g=None):
	...
	s.Suffixes.set(s.TypeSuffix, s.Chain, s.IterationSuffix, s.ConditionSuffix, s.ComputationInfix, s.MatchSuffix, s.Decomposition, s.Access, s.Slice, s.Invocation, s.InfixInvocation)
